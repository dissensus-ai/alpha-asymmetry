...
(intercept $t = -0.86$ on in-position weeks). Transaction costs modestly worsen
...
\toprule
Test Year & Selected Threshold & OOS Return & OOS Sharpe & Hit Rate & Episodes \\
\midrule
2018 & 1.25 & 0.00\% & 0.00 & --- & 0 \\
2019 & 1.25 & 0.00\% & 0.00 & --- & 0 \\
2020 & 1.25 & 2.74\% & 0.88 & 50.0\% & 1 \\
2021 & 1.25 & 0.00\% & 0.00 & --- & 0 \\
2022 & 1.25 & 0.00\% & 0.00 & --- & 0 \\
2023 & 1.25 & 0.00\% & 0.00 & --- & 0 \\
2024 & 1.25 & 0.00\% & 0.00 & --- & 0 \\
2025 & 1.25 & 0.00\% & 0.00 & --- & 0 \\
\midrule
\textbf{Pooled OOS} & --- & \textbf{2.74\%} & \textbf{0.32} & \textbf{50.0\%} & \textbf{1} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: The threshold is selected using data through the prior year, then applied on one sequential OOS state path. Episodes are newly opened directional holdings, not position-change pairs. Pooled return is cumulative ($0.35\%$ annualized); 2025 ends in August. With one episode, these metrics are descriptive only.}
\end{table}

The corrected training windows select 1.25 throughout. The strategy opens one OOS episode, in 2020, and remains flat in the other test years. Its 2.74\% pooled gain therefore cannot be interpreted as evidence of skill: it is one realized path from one holding episode.
...
\begin{tabular}{@{}lccccc@{}}
\toprule
Threshold & Return & Sharpe & MDD & Episodes & Hit Rate \\
\midrule
0.50 & $-$6.83\% & $-$0.12 & $-$22.02\% & 25 & 46.1\% \\
0.75 & $-$7.57\% & $-$0.17 & $-$14.29\% & 15 & 47.3\% \\
1.00 & $-$1.68\% & $-$0.08 & $-$5.46\% & 4 & 43.8\% \\
1.25 & 2.74\% & 0.28 & $-$0.86\% & 1 & 50.0\% \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\textbf{Threshold Selection.} The full-sample grid is non-monotonic. Three thresholds lose money; 1.25 earns 2.74\% from a single episode. Selecting that isolated outcome after seeing the full sample would be data snooping, not robustness. The prespecified 0.75 baseline remains the confirmatory result, and the walk-forward separately freezes each year's threshold from prior data.
...
\midrule
Intercept ($\alpha$) & $-$0.00014 & $-$0.55 & $-$0.00145 & $-$0.86 \\
Carry ($\beta_1$) & 0.0154 & 0.52 & 0.325 & 1.40 \\
Momentum ($\beta_2$) & $-$0.0470 & $-$2.09 & $-$0.824 & $-$3.53 \\
Dollar ($\beta_3$) & 0.0128 & 0.57 & 0.146 & 0.96 \\
\midrule
$R^2$ & \multicolumn{2}{c}{0.010} & \multicolumn{2}{c}{0.187} \\
$F$-statistic & \multicolumn{2}{c}{2.06} & \multicolumn{2}{c}{6.14} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: Newey-West HAC standard errors with 4 lags. The full sample includes flat weeks; the in-position sample is the effective exposure sample. The intercept is not significant in either regression. The negative momentum loading is statistically significant in this specification, but the factors are single-market proxies and the in-position sample remains small.}
\end{table}

\textbf{No Evidence of Positive Residual Alpha.} The intercept is negative and statistically indistinguishable from zero in both specifications ($t = -0.55$ full sample; $t = -0.86$ on in-position weeks). This is evidence against a positive residual-alpha claim, not proof that the population intercept is exactly zero.

\textbf{Factor Loadings and Effective Sample.} The corrected longer holds raise the effective sample to 55 weeks. The negative momentum loading is material in both regressions, while carry and dollar loadings are not significant. Because the factors are proxies and the exposed sample is small, this is a specification check rather than a definitive factor decomposition.
