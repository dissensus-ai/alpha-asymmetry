...
skewness-threshold strategy loses 7.57\% cumulative gross over the decade (15
directional holding episodes, Sharpe $-0.17$), is nearly inert in walk-forward
...
($\beta = -0.82$, $t = -3.73$ on in-position weeks), absent before correction,
indicating that while invested the rule behaves largely as a short momentum
position rather than as an asymmetry harvester. Transaction costs modestly worsen
an already negative gross result; a positive-cost break-even level is therefore
not defined.
Data-snooping corrections complete the null: White's Reality Check ($p = 0.15$)
and Hansen's SPA ($p = 0.26$) find no strategy in a 13-candidate universe that
outperforms the zero-return benchmark---the best realized mean return belongs to
a seeded random candidate. The weekly-return GPD shape parameter is imprecisely estimated ($\xi = -0.25$),
...
\begin{itemize}
    \item \textbf{Tail Alpha ($\alpha_{\text{tail}}$)}: Captures extreme daily price movements without discarding their direction. Formally, $\alpha_{\text{tail},t} = \mathbf{1}_{|r_t| > q_{0.95,t}} \cdot r_t$, where $q_{0.95,t}$ denotes the 95th percentile of absolute daily returns over the trailing 252 trading days (with 60 observations required during warm-up). The Friday observation is retained in the weekly signal panel.
...
\end{equation}
where $AI_t$ is the contemporaneous asymmetry index. Because $AI_t \geq 0$, the formula spans 1--2 gross-notional units; the previously stated 0.5-unit lower bound was unreachable. A missing AI uses the neutral one-unit size. Size is fixed at entry and is not reset or resized by subsequent same-direction signals.
...
\begin{itemize}
    ...
    \item Rebalancing: none within an episode; changes occur only on entry, reversal, conflict, or expiry
...
Monday opening prices are not present in the dataset, so the backtest does not claim Monday-open execution. The Friday-close proxy is an internally consistent bar-to-bar convention, not evidence that the displayed mid-price could have been filled after observing that exact close. All headline, benchmark, walk-forward, factor, cost, and data-snooping returns use the same single-lag convention.
...
\textbf{Skewness.} Under the i.i.d.\ $t$-test, tail and coverage alphas show large-magnitude skewness of opposite signs. But the alphas are serially dependent; pricing, coverage, and hedge have Ljung-Box statistics ranging from 72.6 to 1716, so the block-bootstrap intervals are the appropriate inferential yardstick. On that yardstick only \emph{coverage} alpha survives: its interval $[1.18, 2.16]$ excludes zero decisively, while the tail-alpha interval $[-3.10, 0.54]$ widens to include zero once dependence is respected (the point estimate is dominated by a small number of clustered exceedance weeks). Fast, pricing, and hedge alpha intervals include zero.
...
\midrule
Asymmetry & $-$7.57\% & 4.20\% & $-$0.173 & $-$0.090 & $-$14.29\% & 15 / 30 \\
...
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: Return = cumulative gross return over 504 weeks; Vol = annualized volatility; MDD = maximum drawdown. A holding episode is one continuous directional position; a direct reversal closes one episode and opens another. Execution legs count entries and exits separately, so a reversal has two legs. Turnover, resizing, and reversals are also recorded in the dated ledgers. All strategies use one timing lag.}
\end{table}

\textbf{Risk-Adjusted Performance.} The corrected asymmetry strategy loses 7.57\% gross, with a Sharpe ratio of $-0.173$, versus a 33.53\% buy-and-hold gain. It is invested for 55 of 504 realized-return weeks. Its low 4.20\% annualized volatility still largely reflects inactivity rather than skill. The 20-week momentum benchmark also loses money, while the simple mean-reversion and buy-and-hold benchmarks are positive.

\textbf{Drawdown Analysis.} The asymmetry strategy's maximum drawdown is $-14.29\%$, only modestly smaller than buy-and-hold's $-15.78\%$ despite far lower exposure. Mean reversion remains more contained at $-4.32\%$.

\textbf{Trading Frequency.} The corrected ledger records 15 asymmetry holding episodes, 30 execution legs, one direct reversal, no resizing, and total absolute turnover of 49.15 units. Momentum has 54 directional episodes because every sign reversal closes one episode and opens another; dividing position-change events by two understated this count. These definitions separate economic holding episodes from executions and turnover.

\textbf{Statistical Significance of Returns.} Stationary-bootstrap inference (expected block length 4 weeks, $B = 2000$) gives an annualized return of $-0.81\%$ with a 95\% interval of $[-3.57\%, 1.48\%]$ and a Sharpe interval of $[-0.76, 0.41]$. The sample estimate is negative and the interval remains too wide to establish a nonzero population mean. Failure to reject zero is not proof that the true effect is exactly absent.
...
\midrule
EUR/JPY & $-$1.48 & 0.01 & $-$0.17 & 1.75 & $-$7.57\% & 33.53\% \\
GBP/USD & 0.82 & 0.01 & $-$0.14 & 1.58 & $-$14.11\% & $-$7.63\% \\
SPY & 0.94 & $-$0.21 & $-$1.09 & 1.85 & 13.18\% & 294.30\% \\
GLD & 0.69 & $-$0.00 & $-$0.19 & 1.32 & $-$13.67\% & 200.97\% \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: Bias-corrected skewness of signed weekly signals ($n = 504$ per market). Strategy returns use the corrected one-lag strategy with 15, 19, 4, and 17 directional holding episodes respectively. SPY uses adjusted prices, explicitly requested from the data provider. Point estimates do not include market-specific block-bootstrap inference or costs; this remains exploratory.}
\end{table}

The distributional signature does not travel: EUR/JPY's negative tail-skew point estimate is positive in all three other markets, while coverage alpha's right skew recurs. Strategy outcomes are likewise unstable: the unchanged rules lose 14.11\% on GBP/USD, gain 13.18\% on SPY while trailing buy-and-hold by a wide margin, and lose 13.67\% on GLD. These exploratory transfers lack pair-specific costs and dependence-aware inference, so they do not establish effects in any market.
...
\textbf{It Is Not Trading Asymmetry.} The corrected factor regression carries a statistically significant \emph{negative} loading on time-series momentum ($\beta_2 = -0.82$, $t = -3.73$, $p = 0.00019$ on in-position weeks; $-0.046$, $p = 0.038$ on the full sample), where under the published specification no loading approached significance. While invested, the strategy behaves close to a one-for-one short momentum position on the pair it trades, and momentum alone explains a fifth of the variance of its in-position returns. A rule that sells strength and buys weakness is taking a documented factor exposure, not harvesting distributional asymmetry, and over this sample that exposure lost money. The negative result is therefore not merely that asymmetry failed to pay; it is that the object being tested was substantially something else.

\textbf{Non-Generalizability.} Cross-market transfer yields erratic results: the identical rules lose 14.11\% on GBP/USD, gain 13.18\% on SPY while trailing buy-and-hold by roughly 281 percentage points, and lose 13.67\% on GLD. The tail signal's negative point estimate also reverses sign in every other market tested. Without asset-specific costs or inference, nothing here establishes generalization.
...
Several limitations warrant acknowledgment. First, 504 weekly observations and only 15 holding episodes provide limited power, especially for tail and trade-level inference. Second, the alpha definitions are fully disclosed here but are ad hoc and may not generalize. Third, the factor exercise uses spot proxies rather than published cross-sectional FX factors; even with 55 in-position weeks, the effective sample is small, and the significant negative momentum loading is descriptive rather than a stable structural estimate. Fourth, costs use stylized spreads and indicative mid-prices, omit financing, and assume end-of-bar execution; no Monday-open or executable bid/ask data are available. Fifth, cross-market transfer omits pair-specific financing, costs, and dependence-aware inference. Finally, Yahoo history can be revised. The original raw snapshots were not committed, so exact reproduction of the archived figures is impossible without the owner's files; the revised pipeline caches local inputs and records hashes and software versions.
...
\midrule
Pre-COVID (2016--2019) & 208 & -3.40 & -0.05 & 0.12 & $-$8.06\% \\
COVID Shock (2020) & 52 & 5.02 & 0.09 & -0.07 & 2.74\% \\
Post-COVID (2021--2025) & 244 & -0.73 & 0.05 & -0.38 & $-$2.15\% \\
\midrule
Low VIX (VIX $<$ 20) & 342 & -3.25 & 0.04 & -0.20 & $-$5.78\% \\
High VIX (VIX $\geq$ 20) & 162 & 0.62 & -0.06 & -0.10 & $-$1.90\% \\
\midrule
Rate Hike Regime (2022--2025) & 191 & -0.71 & -0.13 & -0.53 & 3.76\% \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: The strategy is run once on the complete chronological calendar; realized weekly returns are then attributed to rows in each regime. VIX is the mean of daily closes within each Friday-ending week. No stateful strategy is rerun on filtered, nonconsecutive dates. Skewness uses the observations in each displayed subsample.}
\end{table}
...
\textbf{Regime Conditioning.} Low- and high-VIX attribution is negative in both groups ($-5.78\%$ and $-1.90\%$). Because these are ex post return attributions rather than separately tradable conditioned strategies, they describe where gains and losses occurred; they do not establish a usable VIX timing rule.
...
\midrule
Zero Cost (Baseline) & 0.0 & $-$7.57\% & $-$0.173 \\
Prime Brokerage & 0.3 & $-$7.63\% & $-$0.174 \\
Institutional & 0.7 & $-$7.70\% & $-$0.176 \\
Retail (Tight) & 1.3 & $-$7.81\% & $-$0.179 \\
Retail (Wide) & 2.0 & $-$7.93\% & $-$0.182 \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: Each execution leg is charged half the round-trip pips in proportion to absolute notional change at that row's indicative price. Financing is excluded; because exposure can exceed one unit and lasts 55 weeks, its omission is a limitation, not assumed immaterial. A positive-cost break-even is not applicable because the zero-cost return is already negative.}
\end{table}

\textbf{Cost Components.} We model spread and impact per dated execution leg. Financing/carry is excluded uniformly because no suitable dated rate or forward series is present; the omission may matter for leveraged positions and should be addressed with owner-approved data in future work.

\textbf{Cost Interpretation.} Execution costs make the result more negative. Gross return is $-7.57\%$; at two pips round-trip, net return is $-7.93\%$. The gross backtest fails first.
...
\midrule
Intercept ($\alpha$) & $-$0.00012 & $-$0.52 & $-$0.00131 & $-$0.84 \\
Carry ($\beta_1$) & 0.0180 & 0.60 & 0.348 & 1.52 \\
Momentum ($\beta_2$) & $-$0.0462 & $-$2.08$^{*}$ & $-$0.823 & $-$3.73$^{***}$ \\
Dollar ($\beta_3$) & 0.0148 & 0.67 & 0.155 & 1.08 \\
\midrule
...
\textbf{A Significant Negative Momentum Loading.} The correction changes this exercise qualitatively, and not in the strategy's favour. Under the published specification no factor loading approached significance: momentum entered at $\beta_2 = -0.007$ ($t = -0.37$, $p = 0.71$) on the full sample and $-0.247$ ($t = -0.51$, $p = 0.61$) on in-position weeks, and the paper concluded that the regression had no power to say anything. With the holding rule and the execution lag corrected, momentum becomes the one factor that loads significantly, and it loads negatively: $\beta_2 = -0.046$ ($t = -2.08$, $p = 0.038$) on the full sample and $\beta_2 = -0.823$ ($t = -3.73$, $p = 0.00019$) on the 55 in-position weeks, where $R^2$ rises from 0.07 to 0.20. Carry and the dollar factor remain insignificant, and the intercept remains indistinguishable from zero.

The in-position estimate is the economically meaningful one: the full-sample coefficient is mechanically attenuated because the dependent variable is exactly zero in 449 of 504 weeks, which shrinks any loading toward zero without saying anything about the exposure the strategy actually carries when it is in the market. Read on the exposure sample, $\beta_2 = -0.82$ says that while invested the strategy behaves close to a one-for-one \emph{short} position in twelve-week time-series momentum on the pair it trades. That is not an incidental correlation but a plausible mechanical consequence of the rule: the short leg fires when pricing alpha is high, which is to say after the price has run above its sixty-day average, so the strategy systematically sells strength and buys weakness. Momentum alone accounts for a fifth of the variance of its in-position returns.

This matters for the paper's central question. An asymmetry strategy that is a disguised short-momentum bet is not harvesting distributional asymmetry; it is taking a known factor exposure, and one that lost money over this sample (the twenty-week momentum benchmark of Table~\ref{tab:backtest} returned $-18.92\%$). Part of the corrected strategy's loss is therefore attributable to a documented risk premium rather than to any failure specific to asymmetry, which sharpens the null: there is no residual alpha to explain, and what return variation exists is largely explained by an exposure the strategy never intended to take.

\textbf{How Far to Press It.} Three qualifications, none of which overturn the finding but all of which bound it. First, the full-sample $p$-value of 0.038 is marginal and would not survive a multiple-testing correction of the kind this paper applies elsewhere: at family size six (three factors in two specifications) a Bonferroni threshold would be $0.008$. The in-position estimate at $p = 0.00019$ clears that bar comfortably, and the finding should be read as resting on it rather than on the full-sample row. Second, Mom is a twelve-week time-series momentum rule on EUR/JPY itself, not the published cross-sectional currency momentum factor of \citet{menkhoff2012currency}; a strong loading of a single-pair strategy on a single-pair momentum rule is less surprising, and less transferable, than a loading on a genuine cross-sectional factor would be. Third, 55 in-position weeks arising from 15 episodes is a small and serially dependent sample, and HAC standard errors adjust the inference without enlarging the evidence. The loading is a robust description of this sample and a hypothesis about the strategy's character, not a structural estimate.
...
\textbf{Bootstrap Implementation.} White's Reality Check and Hansen's SPA employ the stationary block bootstrap of \citet{politis1994stationary} with expected block length $\ell = 4$ weeks. The 13 candidates comprise five simplified asymmetry rules, the full rule, three momentum rules, two mean-reversion rules, always-long, and one seeded random candidate. Bootstrap replications: 1,000. The formal benchmark is zero weekly return.

\textbf{White's Reality Check.} Following \citet{white2000reality}, we test the candidate universe against a zero-return benchmark. The Reality Check does not reject the null of no superior strategy ($RC = 0.02$, $p = 0.15$). The seeded random position sequence is one prespecified candidate, not the formal benchmark; it happens to have the highest realized sample mean.

\textbf{Hansen's SPA Test.} The Superior Predictive Ability test \citep{hansen2005test} agrees ($SPA = 1.90$, $p = 0.26$): no candidate demonstrates superior predictive ability over the zero-return benchmark at conventional levels.
...
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: Candidates: simplified asymmetry (5), full two-sided asymmetry (1), momentum (3), mean reversion (2), always-long (1), and a seeded random candidate (1). The formal benchmark is zero weekly return. All candidates use one execution lag. Stationary bootstrap: 1,000 replications, expected block length 4 weeks.}
\end{table}

\textbf{Interpretation.} The tests find no statistically superior candidate in this prespecified universe. This is a failure to reject superiority, not proof that every possible strategy has zero expected return. The random candidate's top sample mean underscores the instability of ranking noisy rules.
...
\begin{enumerate}
    ...
    \item The corrected asymmetry strategy loses 7.57\% gross (15 holding episodes, Sharpe $-0.17$), with bootstrap intervals that include zero; walk-forward selection opens one episode in eight test years
    \item Execution costs monotonically worsen the already negative gross result; omitted financing and end-of-bar execution remain limitations
...
\paragraph{AI Assistance.} Claude (Anthropic) assisted with manuscript drafting, code review, and statistical exposition. All research design, data analysis, and interpretation are solely the author's.
